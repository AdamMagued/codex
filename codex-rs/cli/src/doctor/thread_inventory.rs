    ...
    if !scan.files.is_empty() {
        check = check
            .issue(
                ...
                .remedy("Start Kim with no state DB present so startup backfill can create it from rollout files."),
        )
            .remediation(
                "Start Kim with no state DB present so startup backfill can create it from rollout files.",
            );
    }
