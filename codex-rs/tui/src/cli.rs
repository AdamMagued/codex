...
pub struct Cli {
    ...
    // Internal controls set by the top-level `kimcli resume` subcommand.
    // These are not exposed as user flags on the base `kimcli` command.
    #[clap(skip)]
    ...
    /// top-level `kimcli resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `kimcli fork` subcommand.
    // These are not exposed as user flags on the base `kimcli` command.
    #[clap(skip)]
    ...
    /// top-level `kimcli fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
