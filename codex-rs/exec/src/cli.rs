...
#[command(
    version,
    override_usage = "kimcli exec [OPTIONS] [PROMPT]\n       kimcli exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Kim outside a Git repository.
...
struct ResumeArgsRaw {
    ...
    // so "kimcli resume --last <prompt>" treats the positional as a prompt, not a session id.
